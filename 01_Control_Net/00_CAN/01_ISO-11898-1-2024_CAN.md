...
&#8194;&#8195;ISO-11898-1: 2024 中定义了五种 CAN Type (Related to [3.5.4](#354-functional-model-of-mac-sub-layer-architecture))：  
...
<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-Protocol_layer_interactions.png width="400px">
...
<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-DLC_Coding.png>
...
<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-MAC_Function.png>
...
&#8194;&#8195;对于 EF 和 OF，按照以上 3.5.4 MAC 子层结构功能描述，EF 和 OF 的构建及收发位于 MAC 子层；个人看法，EF 和 OF 的 bit 流，由 MAC 构建并完成收发过程，且不会往上传输，但是“错误状态”和“过载状态”的信息应可以在上层与 MAC 子层间进行传递。
...
<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-MAC_DF.png>

<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-MAC_RF.png>

### 3.5.6 &#8194;CAN Frame Specification
...
>
>&#8194;&#8194;&#8195;- the passive error flag shall consist of six consecutive recessive bits unless itis overwritten by dominant bits from other nodes. 
...
&#8194;&#8195;个人看法：以上说明，针对的是同一时间段发送错误帧的节点，即假设 A 节点检测到位错误发送主动错误帧，导致 B 和 C 节点检测到填充错误而发送主动错误帧，则 A 节点会完成 Error flag + Error Delimiter 的连续过程，B 和 C 需要按上述过程完成错误帧的发送，本质是因为 A 与 B / C 发送时段已经不同；关于上述过程的可能解释：发送连续 6 位显性位会导致时钟不同步，检测隐性位相当于对齐时钟，后面再一起发送，确保同时完成错误界定符的发送。
...
>
>&#8194;&#8194;&#8195;- LLC requested OF: This OF isrequested by the LLC sub-layer to indicate an internal overload situation;
>
...
<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-Inter_frame_space_for_nodes.png>

<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-Inter_frame_space_for_error_passive_nodes.png>
...
### 3.5.7 &#8194;MAC frame coding

#### 3.5.7.1 &#8194;Coding requirements
...
&#8194;&#8195;NRZ 编码能够实现极高的波特率，同时又可将辐射保持在限制范围内，但 NRZ 编码不是自同步的（曼彻斯特-Manchester 编码具备自同步机制），即不具有任何同步属性，如果较长时间内没有发生电平变化，则接收方将不能保持同步；
...
#### 3.5.7.3 &#8194;Fixed bit stuffing

>&#8194;&#8195;In the CRC field of FD frames, the stuff bits shall be inserted at fixed positions; they are called fixed stuff bits.
>
>&#8194;&#8195;Fixed bit stuffing shall be applied in the control field, data field and FCRC sequence of XL frames.

&#8194;&#8195;该部分内容会在 CAN-FD 及 CAN-XL 中介绍。 

### 3.5.8 &#8194;Frame validation

#### 3.5.8.1 &#8194;Behaviour when error signalling is enabled
...
>&#8194;&#8195;**Transmitter**
>
>&#8194;&#8195;The frame shall be valid for a transmitter if there is no error until the end of EOF. If a frame is corrupted, recovery shall be processed as described in.
...
#### 3.5.8.2 &#8194;Behaviour when error signalling is disabled
...
### 3.5.9 &#8194;Order of bit transmission
...
>
>&#8194;&#8195;An error-active node mav access the bus as soon as the bus is idle;
>
...
#### 3.5.10.2 &#8194;Transmission of MAC frame

>&#8194;&#8195;MAC frames can be started when the node is allowed to access the bus according to # [3.5.10.2](#35102-transmission-of-mac-frame).
>
...
#### 3.5.10.3 &#8194;Identifier based arbitration
...
#### 3.5.10.5 &#8194;Protocol exception event

&#8194;&#8195;CAN nodes detect a protocol exception event as it is specified in # [3.5.6.1.1.3](#356113-control-field). As a reaction to the protocol exception event, the error counters shall not be changed, the hard synchronization shall be enabled, the node shall send recessive bits and shall enter the busre-integration state.

### 3.5.11 &#8194;MAC data consistency
...
### 3.5.14 &#8194;Error handling and overload handling

#### 3.5.14.1 &#8194;Control of error signalling

>&#8194;&#8195;There shall be a configuration parameter to decide whether error signalling is enabled or is disabled.

#### 3.5.14.2 &#8194;Error detection
...
#### 3.5.14.3 &#8194;Behaviour when error signalling is enabled

#### 3.5.14.3.1 &#8194;Error handling
...
>
>&#8194;&#8195;Dominant bits seen between the CRC delimiter and the start of the EF shall not be treated as errors.
...
#### 3.5.14.3.1 &#8194;Overload signalling
