...
&#8194;&#8194;&#8195;Bit rate 对信息传输速率（传信率）的度量，每秒能够传输的二进制位的数量，单位是 bit per second (bps)。

&#8194;&#8194;&#8195;Baud is the unit for symbol rate or modulation rate in symbols per second or pulses per second. 对符号传输速率（传符号率）的一种度量，单位时间内信号的变化次数或符号更改的速度，单位是 baud。
...
&#8194;&#8194;&#8195;在 CAN 总线中，比特率 = 波特率: 即每个信号符号只编码 1 比特的数据，每个信号变化都表示一个比特（NRZ 编码）。

&#8194;&#8195;**Time quantum**

<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-Bit_Time.png>

<img src=https://github.com/ONEOKCAT/Vehicle_Notes/blob/main/INSET/CAN-Node_Clock.png>

&#8194;&#8194;&#8195;位时间：表示一个二进制位在总线上传输时所需要的时间。

&#8194;&#8194;&#8195;晶振时钟周期：是由单片机振荡器的晶振频率决定的，指的是振荡器每震荡一次所消耗的时间长度，也是整个系统中最小的时间单位（T_q.min）。
...
>
>&#8194;&#8194;&#8195;The minimum time quantum (T_q.min) shall be one node clock period long. 
>
>&#8194;&#8194;&#8195;The length of T_q shall be an integer multiple m of the minimum time quantum T_q.min with m as the value of the prescaler.

>&#8194;&#8194;&#8195;Harmonization of bit time configurations in all nodes of the system is facilitated using harmonized node clocks. It is recommended to use node clock frequencies of 20 MHz, 40 MHz, or 80 MHz for CAN FD applications. For CAN XL applications with a data bit rate of up to 20 Mbit/s, 160 MHz are recommended.
>
...
&#8194;&#8194;&#8195;- **Sync_Seg / 同步段**：CAN 网络中的所有节点，在接收一位数据时，以此段作为位起始的参考点，进行跳变沿的检测，统计跳变沿基于 SS 段的偏移，然后进行位时序的调整，使接收趋于同步（跳变沿在理想情况下应出现在 SS 段）。

&#8194;&#8194;&#8195;需注意，进行位时序调整的条件是检测到跳变沿，若无跳变沿则不进行调整，因此为了避免多个相同连续位出现导致位时序得不到调整，产生不同步的情况，CAN 增加了填充位的概念，
...
&#8194;&#8194;&#8195;- **Phase_Seg 1, Phase_Seg 2 / 相位缓冲段 1 & 2**： 当跳变沿信号没有落在同步段内时，表明发送方和接收方的位时间存在差异，需要进行补偿，通常会采用延长 Phase_Seg 1 或缩短 Phase_Seg 2 的方式。
