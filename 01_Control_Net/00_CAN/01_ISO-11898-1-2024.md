...
&#8194;&#8195;对于 CAN 发送节点而言，为检测 Bit Error，采用了回读（回采）机制，即传输差分电平后，需要回读 CAN Bus上的差分电平，并进行对比。在“回读”过程中，CAN 收发器经历了两次转换：Transmitter 将 0 和 1 数字电平序列转换差分电平；Receiver 将差分电平转换 0 和 1 数字电平序列。因为转换是需要时间的，必然存在物理延迟（Loop Delay / 环路延迟）。
